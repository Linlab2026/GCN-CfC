...
Do not install or change TensorFlow in the existing global/conda environment just for this script. If a TensorFlow-capable environment is required, create it inside this repository folder, for example `<repo-root>\.venv_cfc_tf`, and install only into that local environment.
