        ...
        for i in range(self.hparams["backbone_layers"]):
            ...
            self.backbone.append(
                tf.keras.layers.Dense(
                    ...
                    name=f"backbone_{i}_dense",
                    kernel_regularizer=tf.keras.regularizers.L2(
                        self.hparams["weight_decay"]
                    ),
                )
            )
            self.backbone.append(tf.keras.layers.Dropout(self.hparams["backbone_dr"],name=f"backbone_{i}_dropout"))

        self.backbone = tf.keras.models.Sequential(self.backbone,name="backbone")
