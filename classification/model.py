...
class GCN(nn.Module):
    def __init__(self, block_num, vocab_protein_size, embedding_size=128, filter_num=32, out_dim=2):
        ...
        # self.classifier = nn.Sequential(
        #     nn.Linear(filter_num * 3 * 2, 512),  # 减少到 512 个节点
        #     nn.ReLU(),
        #     nn.Dropout(0.1),
        #     nn.Linear(512, 256),  # 减少到 256 个节点
        #     nn.ReLU(),
        #     nn.Dropout(0.1),
        #     nn.Linear(256, out_dim)
        # )
        self.classifier = nn.Sequential(
            nn.Linear(filter_num * 3 * 2, 256),  # 减少到 256 个节点
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.Linear(256, 128),  # 减少到 128 个节点
            nn.ReLU(),
            nn.Dropout(0.1),
        ...
        # self.classifier = nn.Sequential(
        #     nn.Linear(filter_num * 3 * 2, 128),  # 减少到 128 个节点
        #     nn.ReLU(),
        #     nn.Dropout(0.1),
        #     nn.Linear(128, 64),  # 减少到 64 个节点
        #     nn.ReLU(),
        #     nn.Dropout(0.1),
