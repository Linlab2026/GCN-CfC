...
class GNNDataset(InMemoryDataset):
    def __init__(self, root, types='train', transform=None, pre_transform=None, pre_filter=None):
        ...
        if types == 'train':
            self.data, self.slices = torch_load_dataset(self.processed_paths[0])
        elif types == 'val':
            self.data, self.slices = torch_load_dataset(self.processed_paths[1])
        elif types == 'test':
            self.data, self.slices = torch_load_dataset(self.processed_paths[2])
